...
% addpath(btkFolder);
dataFolder = 'C:\Users\Ganglabor\Documents\automatisiertenEvents\Matlab\Daten';
...
for subj= 1:size(subjects,1)
    ...
    if ~isempty(dataStr)
        ...
        for s = 1:length(side)
            ...
            [nonfiltered_markers,filtered_markers,pelvicMk] = getMarkers(btkData,f,side{1,s},FootMarkersName);
            
            %% Get gait events measured using forceplates
            [mFS,mFO] = getMeasuredGaitEvents(btkData,n,str2num(fpCell{1,1}));
            
            %% sole angle
            [soleAng_beforeFS,soleAng_midStance] = getSoleAngle(btkData,f,side{1,s},mFS,mFO);
            ...
            [varusAng_beforeFS,varusAng_beforeFO,varusAng_midStance] = getVarusAngle(DMT1,DMT5,f,mFS,mFO);
            ...
            for iMk = 1:length(FootMarkersName)
                ...
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Gh,FO_Gh,f,side{1,s});
                results_norm.FS_error_Ghoussayni((subj-1)*2+s,iMk) = errorFS;
                results_norm.FO_error_Ghoussayni((subj-1)*2+s,iMk) = errorFO;

                %% Zeni
                [FS_Zeni,FO_Zeni] = Zeni(filtered_markers(:,:,iMk),pelvicMk,gaitAxis,f);
                
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Zeni,FO_Zeni,f,side{1,s});
                ...
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Des,FO_Des,f,side{1,s});
                ...
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Hsue,FO_Hsue,f,side{1,s});
                ...
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Hreljac,FO_Hreljac,f,side{1,s});
...
for subj= 1:size(subjects,1)
    ...
    if ~isempty(dataStr)
        ...
        for s = 1:length(side)
            ...
            %% sole angle
            [soleAng_beforeFS,~] = getSoleAngle(btkData,f,side{1,s},mFS,mFO);
            results_patients.soleAngle_beforeFS((subj-1)*2+s,1) = soleAng_beforeFS;
            
            %% Varus/valgus (varus >0)
            DMT1 = filtered_markers(:,:,8);
            DMT5 = filtered_markers(:,:,9);
            [varusAng_beforeFS,varusAng_beforeFO,~] = getVarusAngle(DMT1,DMT5,f,mFS,mFO);
...
for subj= 1:size(subjects,1)
    ...
    if ~isempty(dataStr)
        ...
        for h = 1:length(hemi)
            ...
            switch(hemi{1,h})
                case 'affected'
                    results_patients.nameSubjects{dipSubj_number+subj+h-1,1} = [dataStr '_affected'];
                ...
                case 'unaffected'
                    results_patients.nameSubjects{dipSubj_number+subj+h-1,1} = [dataStr '_unaffected'];
                    if trial_aff == trial_unaff
                        c3dPath = [subjectFolder '\' dataStr '.c3d'];
                    else
            ...
            %% load, rotate, filter markers
            [nonfiltered_markers,filtered_markers,pelvicMk] = getMarkers(btkData,f,side,FootMarkersName);
            
            %% Get gait events measured using forceplates
            [mFS,mFO] = getMeasuredGaitEvents(btkData,n,str2num(fpCell{1,1}));
            
            %% sole angle
            [soleAng_beforeFS,~] = getSoleAngle(btkData,f,side,mFS,mFO);
            results_patients.soleAngle_beforeFS(dipSubj_number+subj+h-1,1) = soleAng_beforeFS;
            
            %% Varus/valgus (varus >0)
            DMT1 = filtered_markers(:,:,8);
            DMT5 = filtered_markers(:,:,9);
            [varusAng_beforeFS,varusAng_beforeFO,~] = getVarusAngle(DMT1,DMT5,f,mFS,mFO);
            results_patients.varusAngle_beforeFS(dipSubj_number+subj+h-1,1) = varusAng_beforeFS;
            results_patients.varusAngle_beforeFO(dipSubj_number+subj+h-1,1) = varusAng_beforeFO;

            for iMk = 1:length(FootMarkersName)
                %% Ghoussayni
                [FS_Gh,FO_Gh] = Ghoussayni(filtered_markers(:,:,iMk),gaitAxis,verticalAxis,n,f);
                
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Gh,FO_Gh,f,side);
                results_patients.FS_error_Ghoussayni(dipSubj_number+subj+h-1,iMk) = errorFS;
                results_patients.FO_error_Ghoussayni(dipSubj_number+subj+h-1,iMk) = errorFO;

                %% Zeni
                [FS_Zeni,FO_Zeni] = Zeni(filtered_markers(:,:,iMk),pelvicMk,gaitAxis,f);
                
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Zeni,FO_Zeni,f,side);
                results_patients.FS_error_Zeni(dipSubj_number+subj+h-1,iMk) = errorFS;
                results_patients.FO_error_Zeni(dipSubj_number+subj+h-1,iMk) = errorFO;
                
                %% Desailly
                [FS_Des,FO_Des] = Desailly(nonfiltered_markers(:,:,1),nonfiltered_markers(:,:,iMk),gaitAxis,verticalAxis,f);
                
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Des,FO_Des,f,side);
                results_patients.FS_error_Desailly(dipSubj_number+subj+h-1,iMk) = errorFS;
                results_patients.FO_error_Desailly(dipSubj_number+subj+h-1,iMk) = errorFO;
                
                %% Hsue
                [FS_Hsue,FO_Hsue] = Hsue(filtered_markers(:,:,iMk),pelvicMk,gaitAxis,f);
                
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Hsue,FO_Hsue,f,side);
                results_patients.FS_error_Hsue(dipSubj_number+subj+h-1,iMk) = errorFS;
                results_patients.FO_error_Hsue(dipSubj_number+subj+h-1,iMk) = errorFO;
                
                %% Hreljac
                [FS_Hreljac,FO_Hreljac] = Hreljac(filtered_markers(:,:,iMk),pelvicMk,gaitAxis,verticalAxis,f);
                
                [errorFS,errorFO] = getError(btkData,mFS,mFO,FS_Hreljac,FO_Hreljac,f,side);
                results_patients.FS_error_Hreljac(dipSubj_number+subj+h-1,iMk) = errorFS;
                results_patients.FO_error_Hreljac(dipSubj_number+subj+h-1,iMk) = errorFO;
