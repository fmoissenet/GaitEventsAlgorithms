...
function [varusAng_beforeFS,varusAng_beforeFO,varusAng_midStance] = getVarusAngle(DMT1,DMT5,f,FS,FO)
    ...
    varusAng_beforeFS = mean(varusAng(FS-f/30+1:FS));

    % mean of the sole angle 5 frames (by 150Hz) before FO
    varusAng_beforeFO = mean(varusAng(FO-f/30+1:FO));
